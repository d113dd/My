        ...
        private:
           ...
           *********************/
            void IMU_pub_(int mode)
            {
                ...
                if(mode == 1)
                {
                    imu_msg.angular_velocity.x = Receive_Data.rx[2] << 8 | Receive_Data.rx[3];
                    imu_msg.angular_velocity.y = Receive_Data.rx[4] << 8 | Receive_Data.rx[5];
                    imu_msg.angular_velocity.z = Receive_Data.rx[6] << 8 | Receive_Data.rx[7];
                }
                else if(mode == 2)
                {
                    imu_msg.linear_acceleration.x = Receive_Data.rx[2] << 8 | Receive_Data.rx[3];
                    imu_msg.linear_acceleration.y = Receive_Data.rx[4] << 8 | Receive_Data.rx[5];
                    imu_msg.linear_acceleration.z = Receive_Data.rx[6] << 8 | Receive_Data.rx[7];
                }
                else if(mode == 3)
                {
                    double roll = Receive_Data.rx[2] << 8 | Receive_Data.rx[3];
                    double pitch = Receive_Data.rx[4] << 8 | Receive_Data.rx[5];
                    double yaw = Receive_Data.rx[6] << 8 | Receive_Data.rx[7];
            ...
            *********************/
            void odom_pub_()
            {
                odom_z_scale_positive= odom_z_scale_negative = 1.5;
                odom_x_scale = odom_y_scale = 1;
                odom_data_();
                Receive_Data.A_speed = float(Robot_Pos.A_encode_d)*25/1560*PI*0.060;
                Receive_Data.B_speed = float(Robot_Pos.B_encode_d)*25/1560*PI*0.060;
                Receive_Data.C_speed = float(Robot_Pos.C_encode_d)*25/1560*PI*0.060;
                Receive_Data.D_speed = float(Robot_Pos.D_encode_d)*25/1560*PI*0.060;

                Receive_Data.X_speed = float(Receive_Data.B_speed + Receive_Data.C_speed)/2.0;
                Receive_Data.Y_speed = (Receive_Data.C_speed - Receive_Data.D_speed)/2.0;
                Receive_Data.Z_speed = (Receive_Data.D_speed - Receive_Data.B_speed)/2.0/0.289;
                ...
                //速度*时间=位移（里程计）
                Robot_Pos.X+=(Receive_Data.X_speed * cos(Robot_Pos.Z) - Receive_Data.Y_speed * sin(Robot_Pos.Z)) * 0.04; //Calculate the displacement in the X direction, unit: m //计算X方向的位移，单位：m
                Robot_Pos.Y+=(Receive_Data.X_speed * sin(Robot_Pos.Z) + Receive_Data.Y_speed * cos(Robot_Pos.Z)) * 0.04; //Calculate the displacement in the Y direction, unit: m //计算Y方向的位移，单位：m
                Robot_Pos.Z+=Receive_Data.Z_speed * 0.04; //The angular displacement about the Z axis, in rad //绕Z轴的角位移，单位：rad 
...
// 
            ...
            void odom_data_()
            {
                ...
                if (start_flag_) 
                {
                    Robot_Pos.A_encode = (Receive_Data.rx[2] << 8 & 0xff00) | Receive_Data.rx[3] & 0xff;
                    Robot_Pos.B_encode = (Receive_Data.rx[4] << 8 & 0xff00) | Receive_Data.rx[5] & 0xff;
                    Robot_Pos.C_encode = (Receive_Data.rx[6] << 8 & 0xff00) | Receive_Data.rx[7] & 0xff;
                    Robot_Pos.D_encode = (Receive_Data.rx[8] << 8 & 0xff00) | Receive_Data.rx[9] & 0xff;
                    last_time_ = now_;
                    start_flag_ = false;
                    return;
                }
